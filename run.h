...
void evolution (double x, double y, int count, int seed, double xx, double yy, double theta, double gamma, int pulse_number);
...
void evolution (double x, double y, int count, int seed, double yy, double theta, double gamma, int pulse_number)
{
	int theta_counter;
	...
	sprintf(filename_3,"E=%.2fI=%.2fP=%.2f_neuron_detail_Average_%d_%.2f.txt",x,y,link_density,count,yy);
	fp3=fopen(filename_3,"w");
	fprintf(fp3,"t\t");
	for(i=0;i<3;i++)
	{
		fprintf(fp3,"layer\tbalance\tVm\tu\tx\t\t");
		if(i!=1)
		{
			for(j=0;j<10;j++)
			{
				fprintf(fp3,"balance\tVm\tu\tx\tcount trace\ttrace\tNMDA\tmGluR7\t\t");
			} 
		 } 
		
		fprintf(fp3,"balance\tVm\tcount trace\ttrace\t\t");
	}
	fprintf(fp3,"\n");

	
	FILE *fp7;
	sprintf(filename_3,"W_t_%d_%.2f.txt",count,yy);
	fp7=fopen(filename_3,"w");
	fclose(fp7);
	
	FILE *fp8;
	
	FILE *fp10;
	sprintf(filename_3,"W_t_I_%d_%.2f.txt",count,yy);
	fp10=fopen(filename_3,"w");
	fclose(fp10);
	...
	sprintf(filename_3,"synapse_record_EM3_EM3_%d_%.2f.txt",count,yy);
	fp11=fopen(filename_3,"w");
	fclose(fp11);

	sprintf(filename_3,"synapse_record_E3_E3_%d_%.2f.txt",count,yy);
	fp12=fopen(filename_3,"w");
	fclose(fp12);

	sprintf(filename_3,"synapse_record_EM3_E3_%d_%.2f.txt",count,yy);
	fp13=fopen(filename_3,"w");
	fclose(fp13);

	sprintf(filename_3,"synapse_record_E3_EM3_%d_%.2f.txt",count,yy);
	fp14=fopen(filename_3,"w");
	fclose(fp14);

	sprintf(filename_3,"synapse_record_EM3_EM1_%d_%.2f.txt",count,yy);
	fp15=fopen(filename_3,"w");
	fclose(fp15);

	sprintf(filename_3,"synapse_record_E3_EM1_%d_%.2f.txt",count,yy);
	fp16=fopen(filename_3,"w");
	fclose(fp16);
	
	sprintf(filename_3,"synapse_record_IM3_EM1_%d_%.2f.txt",count,yy);
	fp17=fopen(filename_3,"w");
	fclose(fp17);
	
	sprintf(filename_3,"synapse_record_EM3_IM3_%d_%.2f.txt",count,yy);
	fp18=fopen(filename_3,"w");
	fclose(fp18);
	
	sprintf(filename_3,"synapse_record_EM3_I3_%d_%.2f.txt",count,yy);
	fp19=fopen(filename_3,"w");
	fclose(fp19);
...
////////////////////////////////////////////////////////////////////////////
	...
	for(t=(int)(100000/delta_t);t<(100000+15000)/delta_t;t++)
	{
		...
		sprintf(filename_3,"W_t_%d_%.2f.txt",count,yy);
		fp7=fopen(filename_3,"a");
		sprintf(filename_3,"W_t_I_%d_%.2f.txt",count,yy);
		fp10=fopen(filename_3,"a");
